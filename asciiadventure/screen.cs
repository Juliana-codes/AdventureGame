...
namespace asciiadventure {
    public class Screen {
        ...
        private Boolean TowardsPlayer(int row, int col, int deltaRow, int deltaCol){ //smarter mobs
            int newRow = row + deltaRow;
            int newCol = col + deltaCol;
            if(Math.Abs(newRow - MC.Row) < Math.Abs(row - MC.Row)){
              return true;
             }
            else if(Math.Abs(newRow - MC.Row) > Math.Abs(row - MC.Row)){
              return false;
             }
            else{
              if(Math.Abs(newCol - MC.Col) < Math.Abs(col - MC.Col)){
              return true;
              }
            else if(Math.Abs(newCol - MC.Col) > Math.Abs(col - MC.Col)){
              return false;
              }
            else{
              return true;
        ...
        public List<Tuple<int, int>> GetLegalMoves(int row, int col) {
            ...
            if (IsLegalMove(row, col, -1, 0) && TowardsPlayer(row, col, -1, 0)) {
                moves.Add(Tuple.Create(-1, 0));
            }
            if (IsLegalMove(row, col, 1, 0) && TowardsPlayer(row, col, 1, 0)){
                moves.Add(Tuple.Create(1, 0));
            }
            if (IsLegalMove(row, col, 0, -1) && TowardsPlayer(row, col, 0, -1)){
